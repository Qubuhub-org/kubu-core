Kubu Fee Recommendation
----------------------------

_last updated for 1.0.0_

The Kubu chain has a relatively low block interval, 1 megabyte blockspace
and aims to provide a cheap means for people to transact. Therefore, the biggest
threat to the Kubu chain as a whole is spam. Kubu uses a transaction fee
and dust disincentive, to combat on-chain spam.

Kubu Core implements a number of defaults into the software that reflect the
...
parametrized. The recommended defaults, as implemented in the Kubu Core
wallet, are:

- **0.01 KUBU per kilobyte** transaction fee
- **0.01 KUBU** dust limit (discard threshold)
- **0.001 KUBU** replace-by-fee increments
...
transaction only has to pay `0.01 / 1000 * 192 = 0.00192` KUBU fee.

## Miner default inclusion policies

The default values for miners to include a transaction in a block has been set
to exactly the recommended fee of **0.01 KUBU/kB.** Dust limits are defined by
...
The default minimum transaction fee for relay is set at **0.001 KUBU/kB**,
...
- The hard dust limit is set at **0.001 KUBU** - outputs under this value are
  invalid and rejected.
- The soft dust limit is set at **0.01 KUBU** - sending a transaction with outputs
  under this value, are required to add 0.01 KUBU for each such output, or else
