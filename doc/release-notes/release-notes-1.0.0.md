##Kubu Core 1.0.0 Release Notes (Initial Release)
...
* Remove occurances of Dogecoin and replace with Kubu where appropriate.
* All Kubu addresses now start with the letter `P`. All other address prefixes Dogecoin uses will be adopted by Kubu for familiarity purposes.
...
* Update `key_tests.cpp` with new Kubu addresses to resolve test failures.
...
* Update Dogecoin address strings to Kubu addresses in `uritests.cpp`
* Change magic numbers in `pow.cpp` and `kubu.cpp` to the correct values which reflect the block height digishield min difficulty block activates
...
* Add new Kubu vector logos.
* Add newer and nicer `kubu.png` logo to splash screen.
* Remove Dogecoin logos `qt/res/icons` folder
* Update the `wallet_bgcoin.png` file to the new Kubu logo
...
* Add `kubu.icns` file
* Update fee strings in GUI from Dogecoin theme to Kubu theme
* Create new `about.png` with a Kubu logo
* Add new design for `paper_wallet.png`
* Remove Dogecoin explorer urls
* Update tx urls in gui to point to the Kubu blockchain explorer explorer.kubucoin.org

####Documentation

* Generate Kubu man pages
* Add `--with-gui` to `build-windows.md` instructions
* Change website url to kubucoin.org
* Change ticker symbol from `PPC` to `KUBU` in `readme.md`, man pages, and `amount.cpp`
* Update `assets-attribution.md`
* Update `gen-manpages.sh` to target the bash shell
* Update readme files to correct urls and Kubu icon/text. Remove references to Kubu in the Dogecoin release notes. Move Dogecoin release notes to new `release-notes/dogecoin` folder.
* Add link to official community Discord
* Update `FAQ.md` to reflect Kubu changes
* Update `README_windows.txt` to point to the Dogecoin wiki (The wiki for Dogecoin is a good guide for Kubu as well)
* Change Dogecoin addresses to Kubu addresses in help text.
