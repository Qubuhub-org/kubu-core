...
Below are some notes on how to build Kubu Core for Windows.
...
Kubu GitHub repository](https://github.com/kubucoindev/kubu/)).
...
If you've downloaded via `git`, do not use `sudo`. Instead prefer something like:
    ...
    cd $HOME
    git clone https://github.com/kubucoindev/kubu.git
...
Note that for WSL v1 the Kubu Core source path MUST be somewhere in the default mount file system, for
example /usr/src/kubu, AND not under, for example, /mnt/d/kubu. 
...
Ensure that this toolchain can build for `posix` (else you may have compilation errors for Kubu's dependencies):
...
way. This will install to `c:\workspace\kubu`, for example:

    make install DESTDIR=/mnt/c/workspace/kubu
