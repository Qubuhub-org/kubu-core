...
{
    const std::string URL_SOURCE_CODE = "<https://kubucoin.org>";
    const std::string URL_WEBSITE = "<https://kubucoin.org>";
    const std::string COPYRIGHT_BITCOIN = strprintf("Copyright (C) %i-%i The Bitcoin Core developers", 2009, COPYRIGHT_YEAR);
    const std::string COPYRIGHT_DOGECOIN = strprintf("Copyright (C) %i-%i The Dogecoin Core developers", 2013, COPYRIGHT_YEAR);
    const std::string COPYRIGHT_KUBU = strprintf("Copyright (C) %i The KUBU Core developers", COPYRIGHT_YEAR);

    return COPYRIGHT_BITCOIN + "\n" +
           COPYRIGHT_DOGECOIN + "\n" +
           COPYRIGHT_KUBU + "\n" +
           "\n" +
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                InitNicknameStateDB(std::min<int64_t>(nBlockTreeDBCache, 1 << 20), false, fReindex || fReindexChainState);
    ...
    connOptions.nAvailableFds = nAvailableFds;
    ...
    // KUBU: Do we need to do any RPC mining init here?
