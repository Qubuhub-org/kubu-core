...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, currentTransaction.getRecipients())
    {
        ...
        address.append("</span>");
        QString nickname;
        if (rcp.isNicknameDestination && !rcp.nicknameDestination.trimmed().isEmpty()) {
            nickname = rcp.nicknameDestination.trimmed();
            if (!nickname.startsWith("@")) {
                nickname.prepend("@");
            }
            nickname = GUIUtil::HtmlEscape(nickname);
        }
        QString nicknameMemo;
        if (rcp.isNicknameDestination && !rcp.nicknameMemo.trimmed().isEmpty()) {
            nicknameMemo = GUIUtil::HtmlEscape(rcp.nicknameMemo.trimmed());
        }
        ...
        {
            if(!nickname.isEmpty()) // nickname + resolved address
            {
                recipientElement = tr("%1 to %2").arg(amount, nickname);
                recipientElement.append(QString(" (%1)").arg(address));
                if (!nicknameMemo.isEmpty()) {
                    recipientElement.append(QString("<br /><span style='color:#666;'>%1 %2</span>")
                                            .arg(tr("Memo:"))
                                            .arg(nicknameMemo));
                }
            }
            else if(rcp.label.length() > 0) // label with address
            {
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::DuplicateAddress:
        ...
        break;
    case WalletModel::NicknameMemoRequired:
        msgParams.first = tr("This nickname payment request requires a memo.");
        msgParams.second = CClientUIInterface::MSG_ERROR;
        break;
    case WalletModel::InvalidNicknameMemo:
        msgParams.first = sendCoinsReturn.reasonCommitFailed.isEmpty()
            ? tr("Invalid nickname memo.")
...
void SendCoinsDialog::updateFeeSectionControls()
{
    ...
    // KUBU: We don't use smart fees in the UI, so don't need to warn they're not available
...
void SendCoinsDialog::updateGlobalFeeVariables()
{
    if (ui->radioSmartFee->isChecked())
    {
        ...
        // KUBU: We manually set height well past the last hard fork here
        ui->confirmationTargetLabel->setText(GetKubuPriorityLabel(nPriority).c_str());
    }
    else
    {
...
void SendCoinsDialog::updateFeeLabel()
{
    ...
    int nPriority = ui->sliderSmartFee->value();
    CFeeRate feeRate = GetKubuFeeRate(nPriority);
    ...
    else
    {
        ...
        // KUBU: We don't use smart fees, so we don't have the data to estimate when it will get in
        ui->labelFeeEstimation->setText("");
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid KUBU address"));
        }
        else // Valid address
        {
            CKeyID keyid;
