...
OverviewPage::OverviewPage(const PlatformStyle *platformStyle, QWidget *parent) :
    ...
    labelLockedText(nullptr),
    labelLocked(nullptr),
    txdelegate(new TxViewDelegate(platformStyle, this))
{
    ui->setupUi(this);
    // Keep overview clean: no decorative wallet logo block.
    ui->label_wallet_bgcoin->clear();
    ui->label_wallet_bgcoin->hide();
    ui->label_wallet_version_overlay->clear();
    ui->label_wallet_version_overlay->hide();

    labelLockedText = new QLabel(tr("Locked:"), this);
    labelLocked = new QLabel(this);
    labelLocked->setFont(ui->labelBalance->font());
    labelLocked->setCursor(Qt::IBeamCursor);
    ...
    ui->gridLayout->addWidget(labelLockedText, 6, 0);
    ui->gridLayout->addWidget(labelLocked, 6, 1);
...
void OverviewPage::UpdateTip()
{
    QStringList tips = {
        ...
        tr("To see ongoing development and contribute, check out the KUBU Core repository on GitHub"),
        tr("Services that claim to double your KUBU are always ponzi schemes")
    };
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    const CAmount lockedBalance = walletModel ? walletModel->getLockedBalance() : 0;
    labelLocked->setText(BitcoinUnits::formatWithUnit(unit, lockedBalance, false, BitcoinUnits::separatorAlways));
