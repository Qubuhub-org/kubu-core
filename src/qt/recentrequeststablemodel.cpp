...
static bool ExtractNicknameFromRequestLabel(const QString& label, QString& nicknameOut)
{
    const QString trimmed = label.trimmed();
    const QString prefix = QString("nickname:");
    if (!trimmed.startsWith(prefix, Qt::CaseInsensitive)) {
        return false;
    }

    QString nickname = trimmed.mid(prefix.size()).trimmed();
    if (nickname.isEmpty()) {
        return false;
    }
    ...
    QAbstractTableModel(parent), walletModel(parent)
{
    ...
    columns << tr("Date") << tr("Name") << tr("Label") << tr("Message") << getAmountTitle();
...
QVariant RecentRequestsTableModel::data(const QModelIndex &index, int role) const
{
    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Name: {
            QString nickname;
            if (ExtractNicknameFromRequestLabel(rec->recipient.label, nickname)) {
                return nickname;
            }
            return QString();
        }
        case Label:
        {
            QString nickname;
            if (ExtractNicknameFromRequestLabel(rec->recipient.label, nickname)) {
                return role == Qt::DisplayRole ? tr("(no label)") : QString();
            }
...
bool RecentRequestEntryLessThan::operator()(RecentRequestEntry &left, RecentRequestEntry &right) const
{
    ...
    case RecentRequestsTableModel::Name: {
        QString leftNickname;
        QString rightNickname;
        ExtractNicknameFromRequestLabel(pLeft->recipient.label, leftNickname);
        ExtractNicknameFromRequestLabel(pRight->recipient.label, rightNickname);
