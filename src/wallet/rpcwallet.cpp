...
enum class NicknameMemoType : unsigned char
{
    NUMERIC = 0x01,
    ALNUM = 0x02,
    UTF8 = 0x03,
...
std::string NicknameMemoTypeToString(const NicknameMemoType type)
{
    switch (type) {
    case NicknameMemoType::NUMERIC:
        return "numeric";
    case NicknameMemoType::ALNUM:
        return "alnum";
    case NicknameMemoType::UTF8:
        return "utf8";
    }
...
bool ParseNicknameMemoType(const std::string& typeName, NicknameMemoType& typeOut)
{
    ...
    if (lowered == "numeric") {
        typeOut = NicknameMemoType::NUMERIC;
        return true;
    }
    if (lowered == "alnum") {
        typeOut = NicknameMemoType::ALNUM;
        return true;
    }
    if (lowered == "utf8") {
        typeOut = NicknameMemoType::UTF8;
        return true;
    }
...
void ValidateNicknameMemoDataOrThrow(const NicknameMemoType type, const std::string& data)
{
    if (data.empty()) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Memo must not be empty");
    }
    if (data.size() > KUBU_NICKNAME_MEMO_MAX_DATA_LEN) {
        throw JSONRPCError(RPC_INVALID_PARAMETER,
                           strprintf("Memo is too long (max %d bytes)", KUBU_NICKNAME_MEMO_MAX_DATA_LEN));
    }
    ...
    case NicknameMemoType::NUMERIC:
        for (const unsigned char c : data) {
            if (!std::isdigit(c)) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Numeric memo may only contain digits 0-9");
            }
        }
        return;
    case NicknameMemoType::ALNUM:
        for (const unsigned char c : data) {
            if (!std::isalnum(c)) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Alnum memo may only contain ASCII letters and digits");
            }
        }
        return;
    case NicknameMemoType::UTF8:
        for (const unsigned char c : data) {
            if (c == 0x00) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "UTF-8 memo cannot contain NUL bytes");
            }
        }
        return;
    }
...
std::vector<unsigned char> BuildNicknameMemoPayload(const std::string& data, const NicknameMemoType type)
{
    ValidateNicknameMemoDataOrThrow(type, data);
    ...
    payload.push_back(static_cast<unsigned char>(data.size()));
...
struct NicknameMemoDecoded
{
    NicknameMemoType type;
    std::string data;
    std::vector<unsigned char> payload;
};

NicknameMemoDecoded DecodeNicknameMemoPayload(const std::vector<unsigned char>& payload)
{
    const size_t headerSize = sizeof(KUBU_NICKNAME_MEMO_MAGIC) + 1 + 1 + 1;
    const size_t minPayloadSize = headerSize + 2;
    if (payload.size() < minPayloadSize) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Memo payload is too short");
    }

    if (!std::equal(KUBU_NICKNAME_MEMO_MAGIC,
                    KUBU_NICKNAME_MEMO_MAGIC + sizeof(KUBU_NICKNAME_MEMO_MAGIC),
                    payload.begin())) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Memo payload magic mismatch");
    }

    const unsigned char version = payload[sizeof(KUBU_NICKNAME_MEMO_MAGIC)];
    if (version != KUBU_NICKNAME_MEMO_VERSION) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Unsupported memo payload version");
    }

    const unsigned char rawType = payload[sizeof(KUBU_NICKNAME_MEMO_MAGIC) + 1];
    const unsigned char dataLen = payload[sizeof(KUBU_NICKNAME_MEMO_MAGIC) + 2];
    const size_t expectedSize = headerSize + dataLen + 2;
    if (payload.size() != expectedSize) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Memo payload length is malformed");
    }

    const std::vector<unsigned char> withoutCrc(payload.begin(), payload.end() - 2);
    const uint16_t expectedCrc = ComputeCrc16Ccitt(withoutCrc);
    const uint16_t providedCrc =
        static_cast<uint16_t>(static_cast<uint16_t>(payload[payload.size() - 2]) << 8 |
                              static_cast<uint16_t>(payload[payload.size() - 1]));
    if (expectedCrc != providedCrc) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Memo payload checksum mismatch");
    }

    NicknameMemoDecoded decoded;
    switch (rawType) {
    case static_cast<unsigned char>(NicknameMemoType::NUMERIC):
        decoded.type = NicknameMemoType::NUMERIC;
        break;
    case static_cast<unsigned char>(NicknameMemoType::ALNUM):
        decoded.type = NicknameMemoType::ALNUM;
        break;
    case static_cast<unsigned char>(NicknameMemoType::UTF8):
        decoded.type = NicknameMemoType::UTF8;
        break;
    default:
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Memo payload type is unknown");
    }

    const size_t dataOffset = headerSize;
    decoded.data.assign(payload.begin() + dataOffset, payload.begin() + dataOffset + dataLen);
    ValidateNicknameMemoDataOrThrow(decoded.type, decoded.data);
    decoded.payload = payload;
    return decoded;
}

std::vector<unsigned char> ExtractNicknameMemoPayloadBytes(const std::string& inputHex)
{
    if (!IsHex(inputHex)) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Memo input must be hex");
    }

    const std::vector<unsigned char> raw = ParseHex(inputHex);
    if (raw.empty()) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Memo input must not be empty");
    }
    ...
    if (!script.GetOp(pc, opcode) || opcode != OP_RETURN ||
        !script.GetOp(pc, opcode, pushed) || pc != script.end()) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid OP_RETURN memo script");
    }
    return pushed;
}

CScript BuildNicknameMemoScript(const std::string& data,
                                const std::string& typeName,
                                std::string* canonicalTypeOut = nullptr)
{
    NicknameMemoType type;
    if (!ParseNicknameMemoType(typeName, type)) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid memo_type. Allowed: numeric, alnum, utf8");
    }

    const std::vector<unsigned char> payload = BuildNicknameMemoPayload(data, type);
    if (canonicalTypeOut) {
        *canonicalTypeOut = NicknameMemoTypeToString(type);
    }
...
UniValue getaccountaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nDEPRECATED. Returns the current KUBU address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"address\"          (string) The account KUBU address\n"
            "\nExamples:\n"
...
UniValue getrawchangeaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new KUBU address, for receiving change.\n"
...
UniValue setaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The KUBU address to be associated with an account.\n"
    ...
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid KUBU address");
...
UniValue getaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The KUBU address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "  \"address\"         (string) a KUBU address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
...
static void SendMoney(const CTxDestination& address,
                      ...
                      CWalletTx& wtxNew,
                      const std::vector<CRecipient>* extraRecipients = nullptr)
{
    ...
    if (extraRecipients) {
        vecSend.insert(vecSend.end(), extraRecipients->begin(), extraRecipients->end());
    }
...
static CBitcoinAddress ResolveActiveNicknameAddress(const std::string& nicknameInput, std::string* normalizedNicknameOut = nullptr)
{
    ...
    std::string candidate = nicknameInput;
    if (!candidate.empty() && candidate[0] == '@') {
        candidate.erase(0, 1);
    }

    std::string normalizedNickname;
    Nicknames::NormalizeNickname(candidate, normalizedNickname);
    if (!Nicknames::IsValidNormalizedNickname(normalizedNickname)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid KUBU address or nickname: ") + nicknameInput);
    }

    NicknameInfo info;
    if (!nicknameDB->ReadNickname(normalizedNickname, info)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Nickname not found");
    }
    if (info.GetStatus(chainActive.Height()) != Nicknames::Status::ACTIVE) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "nickname is not active");
    }

    CBitcoinAddress payoutAddress(info.payoutAddress);
    if (!payoutAddress.IsValid()) {
        throw JSONRPCError(RPC_MISC_ERROR, "Indexed nickname payout address is invalid");
    }

    if (normalizedNicknameOut) {
        *normalizedNicknameOut = normalizedNickname;
    }
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"            (string, required) The KUBU address to send to.\n"
            ...
            "                             The recipient will receive less KUBU than you enter in the amount field.\n"
            "\nResult:\n"
...
UniValue sendtonickname(const JSONRPCRequest& request)
{
    if (!EnsureWalletIsAvailable(request.fHelp))
        return NullUniValue;
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 7)
        throw runtime_error(
            "sendtonickname \"nickname\" amount ( \"comment\" \"comment_to\" subtractfeefromamount \"memo\" \"memo_type\" )\n"
            "\nSend an amount to an active nickname's resolved payout address.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"nickname\"           (string, required) The active nickname to send to.\n"
            "2. \"amount\"             (numeric or string, required) The amount in " + CURRENCY_UNIT + " to send. eg 0.1\n"
            "3. \"comment\"            (string, optional) A comment used to store what the transaction is for.\n"
            "4. \"comment_to\"         (string, optional) A comment to store the recipient name.\n"
            "5. subtractfeefromamount  (boolean, optional, default=false) The fee will be deducted from the amount being sent.\n"
            "6. \"memo\"               (string, optional) Optional nickname memo/tag attached as KMEM1 OP_RETURN payload.\n"
            "7. \"memo_type\"          (string, optional, default=\"utf8\") Memo type: \"numeric\", \"alnum\", or \"utf8\".\n"
            "\nResult:\n"
            "\"txid\"                  (string) The transaction id.\n"
            "\nExamples:\n"
            + HelpExampleCli("sendtonickname", "\"kubu_dev\" 0.1")
            + HelpExampleCli("sendtonickname", "\"kubu_dev\" 0.1 \"donation\" \"seans outpost\"")
            + HelpExampleCli("sendtonickname", "\"kubu_dev\" 0.1 \"\" \"\" true \"12345678\" \"numeric\"")
            + HelpExampleRpc("sendtonickname", "\"kubu_dev\", 0.1, \"donation\", \"seans outpost\"")
        );

    LOCK2(cs_main, pwalletMain->cs_wallet);

    std::string normalizedNickname;
    CBitcoinAddress payoutAddress = ResolveActiveNicknameAddress(request.params[0].get_str(), &normalizedNickname);
    ...
    if (request.params.size() > 2 && !request.params[2].isNull() && !request.params[2].get_str().empty())
        wtx.mapValue["comment"] = request.params[2].get_str();
    wtx.mapValue["nickname_to"] = normalizedNickname;
    if (request.params.size() > 3 && !request.params[3].isNull() && !request.params[3].get_str().empty())
        wtx.mapValue["to"]      = request.params[3].get_str();
    else
        wtx.mapValue["to"]      = normalizedNickname;

    bool fSubtractFeeFromAmount = false;
    if (request.params.size() > 4)
        fSubtractFeeFromAmount = request.params[4].get_bool();

    std::vector<CRecipient> extraRecipients;
    if (request.params.size() > 5 && !request.params[5].isNull()) {
        const std::string memoData = request.params[5].get_str();
        if (!memoData.empty()) {
            const std::string memoType = request.params.size() > 6 && !request.params[6].isNull()
                ? request.params[6].get_str()
                : "utf8";
            std::string canonicalMemoType;
            const CScript memoScript = BuildNicknameMemoScript(memoData, memoType, &canonicalMemoType);
            extraRecipients.push_back({memoScript, 0, false});
            wtx.mapValue["nickname_memo"] = memoData;
            wtx.mapValue["nickname_memo_type"] = canonicalMemoType;
        }
    }

    EnsureWalletIsUnlocked();

    SendMoney(payoutAddress.Get(), nAmount, fSubtractFeeFromAmount, wtx, &extraRecipients);

    return wtx.GetHash().GetHex();
}

UniValue encodenicknamememo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2) {
        throw runtime_error(
            "encodenicknamememo \"memo\" ( \"memo_type\" )\n"
            "\nEncode memo/tag into KMEM1 payload for nickname payments.\n"
            "\nArguments:\n"
            "1. \"memo\"       (string, required) Memo/tag data\n"
            "2. \"memo_type\"  (string, optional, default=\"utf8\") One of: \"numeric\", \"alnum\", \"utf8\"\n"
            "\nResult:\n"
            "{\n"
            "  \"memo\": \"...\",\n"
            "  \"memo_type\": \"...\",\n"
            ...
            "}\n"
            "\nExamples:\n"
            + HelpExampleCli("encodenicknamememo", "\"12345678\" \"numeric\"")
            + HelpExampleRpc("encodenicknamememo", "\"12345678\", \"numeric\"")
        );
    }

    const std::string memoData = request.params[0].get_str();
    const std::string memoTypeName = request.params.size() > 1 ? request.params[1].get_str() : "utf8";
    NicknameMemoType memoType;
    if (!ParseNicknameMemoType(memoTypeName, memoType)) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid memo_type. Allowed: numeric, alnum, utf8");
    }

    const std::vector<unsigned char> payload = BuildNicknameMemoPayload(memoData, memoType);
    const std::string canonicalMemoType = NicknameMemoTypeToString(memoType);
    const CScript script = CScript() << OP_RETURN << payload;

    UniValue result(UniValue::VOBJ);
    result.pushKV("memo", memoData);
    result.pushKV("memo_type", canonicalMemoType);
    result.pushKV("payload_hex", HexStr(payload));
    result.pushKV("script_hex", HexStr(script.begin(), script.end()));
    result.pushKV("payload_bytes", static_cast<int>(payload.size()));
    return result;
}

UniValue decodenicknamememo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1) {
        throw runtime_error(
            "decodenicknamememo \"payload_or_script_hex\"\n"
            "\nDecode KMEM1 payload (or OP_RETURN script containing it) for nickname payments.\n"
            "\nArguments:\n"
            "1. \"payload_or_script_hex\"  (string, required) KMEM1 payload hex or full OP_RETURN script hex\n"
            "\nResult:\n"
            "{\n"
            "  \"memo\": \"...\",\n"
            "  \"memo_type\": \"...\",\n"
            ...
            "}\n"
            "\nExamples:\n"
            + HelpExampleCli("decodenicknamememo", "\"4b4d454d310101083132333435363738....\"")
            + HelpExampleRpc("decodenicknamememo", "\"4b4d454d310101083132333435363738....\"")
        );
    }

    const std::vector<unsigned char> payload = ExtractNicknameMemoPayloadBytes(request.params[0].get_str());
    const NicknameMemoDecoded decoded = DecodeNicknameMemoPayload(payload);

    UniValue result(UniValue::VOBJ);
    result.pushKV("memo", decoded.data);
    result.pushKV("memo_type", NicknameMemoTypeToString(decoded.type));
    result.pushKV("payload_hex", HexStr(decoded.payload));
    result.pushKV("payload_bytes", static_cast<int>(decoded.payload.size()));
    result.pushKV("version", static_cast<int>(KUBU_NICKNAME_MEMO_VERSION));
    return result;
}
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"address\",            (string) The KUBU address\n"
...
UniValue signmessage(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw runtime_error(
            ...
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"address\"         (string, required) The KUBU address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The KUBU address for transactions.\n"
...
UniValue sendfrom(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 6)
        throw runtime_error(
            ...
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a KUBU address."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            ...
            "                       the spend.\n"
            "2. \"toaddress\"         (string, required) The KUBU address to send funds to.\n"
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw runtime_error(
            ...
            "2. \"amounts\"             (string, required) A json object with addresses or active nicknames and amounts\n"
            "    {\n"
            "      \"destination\":amount   (numeric or string) The KUBU address or active nickname is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            "                           Those recipients will receive less KUBU than you enter in their corresponding amount field.\n"
    ...
    BOOST_FOREACH(const string& name_, keys)
    {
        ...
        if (!address.IsValid()) {
            address = ResolveActiveNicknameAddress(name_);
        }

        if (setAddress.count(address))
            throw JSONRPCError(RPC_INVALID_PARAMETER, string("Invalid parameter, duplicated destination: ")+name_);
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
    {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "Each key is a KUBU address or hex-encoded public key.\n"
            "If 'account' is specified (DEPRECATED), assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keys\"         (string, required) A json array of KUBU addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) KUBU address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) DEPRECATED. An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"address\"         (string) A KUBU address associated with the keys.\n"
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"address\",    (string) The KUBU address of the transaction. Not present for \n"
    ...
    if (request.fHelp || request.params.size() > 2)
        throw runtime_error(
           ...
           "[                     (json array of string)\n"
            ...
            "         \"address\" : \"address\",          (string) The KUBU address involved in the transaction\n"
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp)
        throw runtime_error(
            ...
            "    \"address\":\"address\",    (string) The KUBU address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    ...
    if (pwalletMain->IsCrypted() && (request.fHelp || request.params.size() != 2))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending KUBU\n"
            "\nArguments:\n"
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    if (!pwalletMain->IsCrypted() && (request.fHelp || request.params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending KUBU\n"
    ...
    StartShutdown();
    return "wallet encrypted; KUBU server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
UniValue lockunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending KUBU.\n"
...
UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 5)
        throw runtime_error(
            ...
            "3. \"addresses\"      (string) A json array of KUBU addresses to filter\n"
            "    [\n"
            "      \"address\"     (string) KUBU address\n"
            "      ,...\n"
            "    ]\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the KUBU address\n"
    ...
    if (request.params.size() > 2 && !request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid KUBU address: ")+input.get_str());
            if (setAddress.count(address))
...
UniValue fundrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
                            ...
                            "   {\n"
                            "     \"changeAddress\"          (string, optional, default pool address) The KUBU address to receive the change\n"
    ...
    if (request.params.size() > 1) {
      if (request.params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid KUBU address");
...
UniValue bumpfee(const JSONRPCRequest& request)
{
    ...
    // Round txSize up to nearest 1kB
    if (txSize % 1024 != 0) {
    ...
    if (totalFee > 0) {
        ...
        // otherwise bump the fee by 1 KUBU.
        else {
...
{ //  category              name                        actor (function)           okSafeMode
    ...
    { "wallet",             "decodenicknamememo",       &decodenicknamememo,       true,   {"payload_or_script_hex"} },
    { "wallet",             "dumpprivkey",              &dumpprivkey,              true,   {"address"}  },
    { "wallet",             "dumpwallet",               &dumpwallet,               true,   {"filename"} },
    { "wallet",             "encodenicknamememo",       &encodenicknamememo,       true,   {"memo","memo_type"} },
    ...
    { "wallet",             "sendtonickname",           &sendtonickname,           false,  {"nickname","amount","comment","comment_to","subtractfeefromamount","memo","memo_type"} },
