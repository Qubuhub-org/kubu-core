...
UniValue importprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
        throw runtime_error(
            "importprivkey \"kubuprivkey\" ( \"label\" ) ( rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"kubuprivkey\"  (string, required) The private key (see dumpprivkey)\n"
...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid KUBU address or script");
    }
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"address_or_nickname\"\n"
            "\nReveals the private key corresponding to 'address'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"address_or_nickname\"   (string, required) The KUBU address or nickname for the private key\n"
            "\nResult:\n"
            "\"key\"                (string) The private key\n"
            "\nExamples:\n"
            + HelpExampleCli("dumpprivkey", "\"myaddress\"")
            + HelpExampleCli("dumpprivkey", "\"kubu_dev\"")
            + HelpExampleCli("importprivkey", "\"mykey\"")
    ...
    if (!address.SetString(strAddress)) {
        std::string normalized = strAddress;
        if (!normalized.empty() && normalized[0] == '@') {
            normalized.erase(0, 1);
        }
        Nicknames::NormalizeNickname(normalized, normalized);

        NicknameStateDB* nicknameDB = GetNicknameStateDB();
        NicknameInfo info;
        if (!nicknameDB || !nicknameDB->ReadNickname(normalized, info)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid KUBU address or nickname");
        }
        if (!address.SetString(info.payoutAddress)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Nickname has invalid payout address");
        }
    }
    CKeyID keyID;
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by KUBU %s\n", CLIENT_BUILD);
