...
/* Dogecoin 1.14.5: set the wallet's discard threshold to 1 KUBU because that's what 97%
 ...
 */
/* Dogecoin 1.14.6: set the wallet's discard threshold to 0.01 KUBU. Very network
...
/*
 * KUBU: Scaled to 1/10th of the recommended transaction fee to make RBF
...
/*
 * KUBU: Creating change outputs at exactly the dustlimit is counter-
...
public:
    ...
    /**
     * KUBU: Get a fee targetting a specific transaction speed.
     */
    CAmount GetKubuPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed);
    /**
     * KUBU: Get a fee targetting a specific transaction speed.
     */
    static CAmount GetKubuPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed, CAmount targetFee);
    /**
    ...
    /**
     * Return the minimum required fee taking into account the
     * floating relay fee and user set minimum transaction fee, but not the KUBU dust fee.
     */
    ...
    //! Get wallet transactions that spend a specific outpoint
    std::set<uint256> GetSpendsForOutpoint(const COutPoint& outpoint) const;
