...
# KUBU TODO Change these urls to kubucoin.org and add a depends subdomain which will do what this does
# Dependencies
...
SIGNER=
VERSION=
url=https://github.com/kubucoindev/kubu
proc=2
mem=2000
...
Standalone script to perform the gitian build of Kubu Core. Perform
...
-u|--url repo       Specify the URL of the repository. Default is https://github.com/kubucoindev/kubu
...
function move_build_files() {
    find build/out -type f -exec mv '{}' $outputDir/kubu-binaries/${VERSION}/ \;
}
...
if [[ $setup == true ]]; then
    git clone https://github.com/kubucoindev/gitian.sigs.git
    git clone https://github.com/kubucoindev/kubu-detached-sigs.git
...
if [[ $build == true ]]; then
    # Make output folder
    mkdir -p $outputDir/kubu-binaries/"$VERSION"

    pushd ./gitian-builder || exit 1

    # Clean kubu git directory because of old caching
    if [ -d inputs/kubu/ ]; then
        echo "Cleaning Kubu directory..."
        rm -rf inputs/kubu/
    fi
    ...
    for descriptor in "${DESCRIPTORS[@]}"; do
        ...
        echo ""
        ./bin/gbuild -j "$proc" -m "$mem" --commit kubu="$COMMIT" --url kubu="$url" ../gitian-descriptors/gitian-"$descriptor".yml  || exit 1
